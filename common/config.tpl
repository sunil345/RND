...
if( !empty($column_left) && !empty($column_right) ){
	...
	$sbcols = 2; 
	if($this->config->get('config_layout_id')==16){
		$sbcols = 2;
	}
	$spans = array( 'full' 			=> array( $sbcols, "8", $sbcols ), /* for layout has left - center - right */
					'center-right'  => array("0", "10",$sbcols ), /* for layout has center - right */
					'center-left'   => array( $sbcols, "10", "0" ), /* for layout has left -center */
