...
<div class="clearfix">
	<table class="form">
		<tr>
			<td>
			<b><?php echo $this->language->get('text_show_module_onlayout'); ?></b>
			</td>
			<td>
...
<div class="alert alert-success"><i><?php echo $this->getLang("text_explain_modules_function");?></i></div>
<div class="theme-layout clearfix">
       ...
       <div class="mass_bottom_top  edit-container clearfix"  data-position="mass_bottom_top">
			<div class="pos">Massbottom Top</div>
			<?php if( isset($layout_modules['mass_bottom_top']) ){  
				foreach( $layout_modules['mass_bottom_top'] as $modulepos ) {
			?>
			<div class="module-pos <?php if( !$modulepos['status']){ ?>mod-disable<?php } ?>" data-position="mass_bottom_top" data-id="<?php echo $modulepos['code']."-".$modulepos['index'];?>">
				<div class="edit-module">
					<a target="_blank" href="<?php echo $this->url->link('module/'.$modulepos['code'],'token='.$token);?>"><b><?php echo $modulepos['title']; ?></b></a><i>[[index:<?php echo $modulepos['index'];?>]</i>
				</div>
			</div>
...
$(".edit-module a").click( function(){
	...
	$('#dialogeditmodule').dialog({
		title: '<?php echo $this->language->get("text_module_setting"); ?>',
		close: function (event, ui) {
			 
		},	
		open: function(){
				 $("#dialogeditmodule iframe").load( function(){  
				 	  $('body', $('#dialogeditmodule iframe').contents() ).find("#menu").hide();
				 	  $('body', $('#dialogeditmodule iframe').contents() ).find("#header").hide();
				 	  $('body', $('#dialogeditmodule iframe').contents() ).find("#footer").hide();
			 	      $('body', $('#dialogeditmodule iframe').contents() ).find(".breadcrumb").hide();
				 } );
		},			
		bgiframe: false,
		width: 1000,
